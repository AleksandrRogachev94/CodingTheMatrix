...
def add(A, B):
    ...
    """
    assert A.D == B.D
    return Mat(A.D, { d:(A[d] + B[d]) for d in A.f.keys() | B.f.keys() })
...
def scalar_mul(M, x):
    ...
    True
    """
    return Mat(M.D, { d:(val * x) for d, val in M.f.items() })
...
def matrix_matrix_mul(A, B):
    ...
    res = Mat((A.D[0], B.D[1]), {})
    # iterate over nonzero elements of A
    for (A_row, A_col), A_val in A.f.items():
        # for each nonzero element of A, multiply it with the corresponding B's columns.
        for B_col in B.D[1]:
            B_val = B[A_col, B_col]
            # don't multiply of B_val is zero (alternatively, find a way to iterate over a specific SPARSE column of B).
            if(B_val != 0):
                res[A_row, B_col] += A_val * B[A_col, B_col]
